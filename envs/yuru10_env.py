"""
yuru10_env.py — yuru9 (best / most natural gait) + LATERAL touch-up.

Goal: reduce lateral foot splay and side-to-side trembling.
Both are LATERAL (side) issues, and linked (large hip swing -> feet splay out + torso sways):
  - c_genis : keep the feet from splaying out too much (keep foot y-position near the hip line, narrow stance)
  - c_roll  : 0.5 -> 1.0 (MEDIUM; stop the trembling / side-tilt; NOT yuru7's harsh 1.5)
Everything else is the SAME as yuru9 (hip-stride, knee, phase-clock... = the well-liked wide stride is preserved).
"""
import os
...
class Yuru10Env(gym.Env):
    ...
    def step(self, action):
        ...
        c_roll = 1.0 * abs(roll)                                  # 0.5 -> 1.0 (stop the trembling)
        # LATERAL FOOT SPREAD penalty: keep foot y-position from opening more than 8cm (narrow stance)
        ...
        odul = (r_faz + r_clear + r_diz + r_stride + r_ileri + r_canli + r_dikz
                - c_kayma - c_lean - c_roll - c_genis - c_zipla - c_yana - c_enerji - c_smooth - c_bilek)
