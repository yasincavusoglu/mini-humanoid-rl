"""
yuru8_env.py — yuru6 (SUCCESSFUL: balanced+upright+symmetric gait) + GENTLE knee/shake touch-up.

yuru7 lesson: I set the knee+anti-shake penalties WAY TOO HARD -> over-constrained -> the robot
gave up and fell at 0.2s (ep_len 11). yuru6 was already very good (0 falls, balanced, upright).
yuru8 = yuru6 REWARD AS-IS + only 3 SMALL additions (small enough not to disturb balance):
  + r_diz  : make the swing knee bend a bit more (SMALL positive reward)
  - c_roll : keep the torso from tilting sideways (SMALL)  -> side-to-side sway
  - c_smooth: sudden action change (SMALL)     -> shake/tremor
(NONE of yuru7's harsh c_yalpa/large coefficients.)
"""
import os
...
class Yuru8Env(gym.Env):
    ...
    def step(self, action):
        ...
        r_clear *= 3.0
        # GENTLE knee-bend (make the swing knee bend a bit more)
        r_diz = 0.0
        ...
        # penalties: yuru6 + GENTLE roll & smooth (NONE of yuru7's harsh stuff)
        c_kayma = 0.6 * (lf_slip + rf_slip)
        c_lean = 1.0 * abs(pitch)
        c_roll = 0.5 * abs(roll)                                   # GENTLE lateral tilt
        c_zipla = 0.4 * abs(vz)
        c_yana = 0.15 * abs(vy)
        c_enerji = 0.0005 * float(np.sum(np.square(action)))
        c_smooth = 0.02 * float(np.sum(np.abs(action - self._prev_a)))   # GENTLE tremor
