"""
yuru11_env.py — yuru10 + 2 TARGETED changes: fix the trembling + limping (leader 60/34).
Design: an adversarially-pruned panel (A/B/C -> synthesis). Collapse risk: low.
  ADD-1 r_adim (REWARD): reward SYMMETRICALLY the swing leg passing the stance leg in x (forward)
                       -> leader left60/right34 -> ~50/50 (limping ends). Being a reward, the robot cannot give up.
  ADD-2 c_rollrate (single small PENALTY): 0.05*|qvel[3]| (roll-RATE, wx) -> trembling / sway is damped directly.
KEPT (deliberately): c_roll 1.0 (proven), r_stride/r_diz (wide stride), c_genis 0.8, c_smooth 0.02 = SAME as yuru10.
"""
import os
...
class Yuru11Env(gym.Env):
    ...
    def step(self, action):
        ...
        lhp = float(self.data.qpos[9]); rhp = float(self.data.qpos[15])
        ...
        r_diz *= 0.6
        r_stride = min(abs(lhp - rhp), 0.8) * 0.6

        # ---- yuru11 ADD-1: SYMMETRIC LEADER reward (swing leg steps FORWARD) ----
        # SAME phase-gating as r_clear/r_diz: not sol_stance -> LEFT in swing, not sag_stance -> RIGHT in swing.
        # Only one guard is active per step; SYMMETRIC across both legs => throwing one leg forward stops paying, alternation is profitable.
        r_adim = 0.0
        if not sol_stance:                                        # LEFT leg in swing
            r_adim += min(max(float(lf_xy[0] - rf_xy[0]), 0.0), 0.10)   # left foot (x=forward) should pass ahead
        if not sag_stance:                                        # RIGHT leg in swing
            r_adim += min(max(float(rf_xy[0] - lf_xy[0]), 0.0), 0.10)   # right foot should pass ahead
        r_adim *= 2.0                                             # max ~0.20 per step (on the order of r_clear/r_ileri)
        ...
        c_roll = 1.0 * abs(roll)                                  # KEPT (proven; NOT B's 0.8)
        c_genis = 0.8 * (max(0.0, abs(float(lf_xy[1])) - 0.08) + max(0.0, abs(float(rf_xy[1])) - 0.08))
        c_zipla = 0.4 * abs(vz)
        c_yana = 0.15 * abs(vy)
        ...
        # ---- yuru11 ADD-2: roll-RATE damping (trembling directly targeted) ----
        # qv[3] = roll-rate (wx). c_roll (angle, 1.0) is KEPT; this only damps the RATE term moderately.
        c_rollrate = 0.05 * abs(float(qv[3]))                     # the ONLY new small penalty (not B's 0.12)

        odul = (r_faz + r_clear + r_diz + r_stride + r_adim + r_ileri + r_canli + r_dikz
                - c_kayma - c_lean - c_roll - c_genis - c_zipla - c_yana
                - c_enerji - c_smooth - c_bilek - c_rollrate)
