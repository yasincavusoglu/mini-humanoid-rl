"""
yuru3_env.py — REAL STEP: make every leg joint work + penalize reward-hacks.

yuru2 problem: the robot moved forward by pushing with the ANKLE and SLIDING
(skating/shuffle); the hip/knee were not doing the real work, the foot was dragging. yuru3 fixes
this with the following terms:

  REWARDS (encourage a real step):
    + forward velocity (saturated)                : keep moving forward
    + single-foot-support                          : the natural phase of walking
    + BENT SWING KNEE (airborne leg's knee)        : a bent knee = a real step (not stiff)
    + HIP STRIDE (one hip forward, one back)        : let the legs open a real stride
    + foot clearance (airborne foot lifts)         : not dragging, lift it
  PENALTIES (reward-hack / cheat):
    - STANCE FOOT SLIP                             : ** main anti-cheat ** (no push-and-slide with the foot)
    - TORSO PITCHING FORWARD/BACK (pitch)          : stand upright; no fall-and-catch walking
    - both feet airborne (flight)                  : no hopping
    - torso vertical bobbing                        : no bob-bob
    - lateral slip / wobble / (low) energy
"""
import os
...
DT = 0.02   # env step duration (4 x 0.005)


class Yuru3Env(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        # qpos joint indices
        self.iLHP, self.iLK = 9, 10     # left hip-pitch, left knee
        self.iRHP, self.iRK = 15, 16    # right hip-pitch, right knee
        ...
        self.max_steps = 1000
        self._steps = 0
        self._prev_lf = np.zeros(2)
        self._prev_rf = np.zeros(2)
    ...
    def step(self, action):
        ...
        R = self._R()
        dik = float(R[2, 2])
        pitch = float(np.arcsin(np.clip(R[0, 2], -1, 1)))       # torso pitching forward/back (rad)
        ...
        lf_xy = self.data.xpos[self._lfb][:2].copy()
        rf_xy = self.data.xpos[self._rfb][:2].copy()
        ...
        lk = float(self.data.qpos[self.iLK]); rk = float(self.data.qpos[self.iRK])    # knee angle (rad, +bent)
        lhp = float(self.data.qpos[self.iLHP]); rhp = float(self.data.qpos[self.iRHP])
        l_sw = not sol; r_sw = not sag                          # swing (airborne) leg
        ...
        # make the swing knee bent (a real step, not stiff)
        r_diz = 0.0
        if l_sw: r_diz += min(max(lk, 0.0), 0.6)
        if r_sw: r_diz += min(max(rk, 0.0), 0.6)
        r_diz *= 0.5
        # hip stride: one hip forward, the other back
        ...
        c_kayma = 1.0 * (lf_slip + rf_slip)      # ** stance foot slip = main cheat penalty **
        c_lean = 1.5 * abs(pitch)                # keep the torso upright (forward/back lean)
        c_ucus = 1.0 if n == 0 else 0.0          # both feet airborne = hopping
        c_zipla = 0.5 * abs(vz)                  # vertical bobbing
        c_yana = 0.15 * abs(vy)
        c_yalpa = 0.02 * float(np.linalg.norm(w))
        c_enerji = 0.0005 * float(np.sum(np.square(action)))

        odul = (r_ileri + r_canli + r_dikz + r_tek + r_diz + r_stride + r_clear
                - c_kayma - c_lean - c_ucus - c_zipla - c_yana - c_yalpa - c_enerji)

        terminated = (z < 0.25) or (dik < 0.5)   # uprightness threshold 0.4->0.5 (stay more upright)
