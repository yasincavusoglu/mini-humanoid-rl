"""
yuru12_env.py — yuru10 (STRAIGHT + balanced base) + the trembling-fix that WORKED in yuru11 + anti-veer.
Goal: straight + calm + wide stride. yuru11 solved the trembling (roll 8.3) but VEERED (r_adim backfired).
  BASE  : yuru10 (walks straight, leader 60/34, roll 12.9, narrow stance 21cm) — AS IS.
  +ADD-1: c_rollrate = 0.05*|qv[3]| (roll-RATE damping) -> trembling drops directly (proven in yuru11).
  +ADD-2: c_drift = 0.25*|qpos[1]| (torso lateral position) -> prevents sideways VEERING; ~0 in straight walking.
  DROPPED: r_adim (in yuru11 it gamed the reward by turning on one leg -> veering). GONE.
Everything else is the SAME as yuru10 (c_roll 1.0, r_stride/r_diz wide stride, c_genis narrow stance, c_smooth).
"""
import os
...
class Yuru12Env(gym.Env):
    ...
    def step(self, action):
        ...
        c_roll = 1.0 * abs(roll)                                  # KEPT (proven)
        ...
        # ---- yuru12 ADD-1: roll-RATE damping (trembling) — proven in yuru11 ----
        c_rollrate = 0.05 * abs(float(qv[3]))                     # qv[3]=roll-rate (wx)
        # ---- yuru12 ADD-2: anti-veer — keep torso from drifting sideways off the x-axis (prevent veering) ----
        c_drift = 0.25 * abs(float(self.data.qpos[1]))           # qpos[1]=torso lateral (y) position; straight->~0

        odul = (r_faz + r_clear + r_diz + r_stride + r_ileri + r_canli + r_dikz
                - c_kayma - c_lean - c_roll - c_genis - c_zipla - c_yana
                - c_enerji - c_smooth - c_bilek - c_rollrate - c_drift)
