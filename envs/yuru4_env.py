"""
yuru4_env.py — yuru3 + LEADING-LEG ALTERNATION.

yuru3 problem: the robot ALWAYS has its LEFT leg forward; the right leg never
comes forward (limping / single-leg step). Solution:
  - if the SAME leg stays FORWARD longer than a certain time -> PENALTY (growing)
  - when the leading leg CHANGES (real alternating stepping) -> BONUS
This way the "left forward -> right forward -> left forward" alternation is enforced.
Everything else is the same as yuru3 (stand upright, bend knee, hip stride, foot slip penalty...).
"""
import os
...
class Yuru4Env(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self._prev_lead = 1     # +1 left forward, -1 right forward
        self._ayni = 0          # how many steps the same leg has been forward
    ...
    def step(self, action):
        ...
        # ---- LEADING-LEG ALTERNATION ----
        dx = float(lf_xy[0] - rf_xy[0])          # left_foot_x - right_foot_x
        if dx > 0.04:   lead = 1                  # left clearly forward
        elif dx < -0.04: lead = -1                # right clearly forward
        else:           lead = self._prev_lead    # transition, keep it
        r_switch = 0.0
        if lead != self._prev_lead:
            self._ayni = 0
            r_switch = 0.6                         # real alternating stepping -> BONUS
        else:
            self._ayni += 1
        self._prev_lead = lead
        c_ayni = 0.05 * max(0, self._ayni - 55)    # ~1.1s same leg forward -> PENALTY (prevent limping)
